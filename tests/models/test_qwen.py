...
from flash_attn.models.qwen import (
    config_from_checkpoint,
    qwen_config_to_gpt2_config,
    remap_state_dict_hf_qwen,
...
@pytest.mark.parametrize("model_name", ["Qwen-7B"])
def test_qwen_state_dict(model_name):
    checkpoint_path = Path(
        os.environ.get("CHECKPOINT_DIR", current_dir.parent.parent / "checkpoints")
    )
    config = qwen_config_to_gpt2_config(config_from_checkpoint(checkpoint_path, model_name))
    ckpt_state_dicts = state_dicts_from_checkpoint(checkpoint_path, model_name)
    pretrained_state_dict = remap_state_dict_hf_qwen(ckpt_state_dicts[0], config)
    model = GPTLMHeadtransformer(config, device="meta")  # Without device='meta' init is very slow
...
@pytest.mark.parametrize("model_name", ["Qwen-7B"])
def test_qwen_optimized(model_name):
    """Check that our implementation of qwen (with all optimizations enabled) matches the
    ...
    ckpt_state_dicts = state_dicts_from_checkpoint(checkpoint_path, model_name)
    pretrained_state_dicts = [remap_state_dict_hf_qwen(s, config) for s in ckpt_state_dicts]
    pretrained_state_dict = combine_state_dicts_tp(pretrained_state_dicts, config)
    model = GPTLMHeadtransformer(config, device=device, dtype=dtype)
...
# torchrun --no_python --nproc_per_node=2 pytest -q -s tests/models/test_qwen.py -k "test_qwen_parallel"
@pytest.mark.parametrize("world_size", [2])
@pytest.mark.parametrize("model_name", ["Qwen-7B"])
def test_qwen_parallel(model_name, world_size):
    """Check that our implementation of qwen (with all optimizations enabled) matches the
    HF implementation: the output of our forward pass in fp16 should be around the same as the HF
    forward pass in fp16, when compared to the HF forward pass in fp32.
    """
...
@pytest.mark.parametrize("model_name", ["Qwen-7B"])
def test_qwen_generation(model_name):
    checkpoint_path = Path(
        os.environ.get("CHECKPOINT_DIR", current_dir.parent.parent / "checkpoints")
    )
...
# torchrun --no_python --nproc_per_node=2 pytest -q -s tests/models/test_qwen.py -k "qwen_parallel_generation"
@pytest.mark.parametrize("world_size", [2])
@pytest.mark.parametrize("model_name", ["Qwen-7B"])
def test_qwen_parallel_generation(model_name, world_size):
