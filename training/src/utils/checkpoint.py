...
def load_checkpoint(path, device="cpu"):
    ...
    if path.is_dir():
        ...
        hf_bin_paths = [p for p in path.glob("pytorch_model*.bin")]
        if len(hf_bin_paths) > 0:
            state_dict = OrderedDict()
            for p in hf_bin_paths:
...
def blockdiag_to_dense_mlp_bert(state_dict):
    ...
    names = {
        ...
        if re.match(
            "bert.encoder.layer.(\d+).(mlp.fc(1|2)|(intermediate|output).dense).weight", name
        )
    }
    for name in names:
...
def interpolate_pos_embedding(
    state_dict, out_seqlen, pos_embedding_name="model.pos_encoder.pe", interleave=False
):
    orig_emb = state_dict["state_dict"][pos_embedding_name]
    assert (
        out_seqlen % orig_emb.shape[1]
    ) == 0, "out_seqlen must be a multiple of the original sequence length"
    reps = [1 for i in orig_emb.shape]
    reps[1] = out_seqlen // orig_emb.shape[1]

    if interleave:
        assert (
            math.isqrt(orig_emb.shape[1]) ** 2 == orig_emb.shape[1]
        ), "interleave only works for square lengths"
        ...
        emb_square_expanded = emb_square.repeat_interleave(
            math.isqrt(reps[1]), axis=1
        ).repeat_interleave(math.isqrt(reps[1]), axis=2)
