...
class SequenceModel(LightningModule):
    ...
    def shared_step(self, batch: Any, batch_idx: int, phase="train"):
        ...
        log_on_step = (
            "eval" in self.cfg and self.cfg.eval.get("log_on_step", False) and phase == "train"
        )
        self.log(
            f"{phase}/loss",
            loss,
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        checkpoint["loops"]["fit_loop"]["epoch_loop.batch_progress"]["total"]["completed"] = (
            checkpoint["loops"]["fit_loop"]["epoch_loop.batch_loop.optimizer_loop.optim_progress"][
                "optimizer"
...
class ParallelSequenceLMModel(SequenceLMModel):
    def __init__(self, cfg, model_cfg=None):
        """If model_cfg is passed, it will take precedence over cfg.model"""
    ...
    def setup(self, stage):
        # setup called after ddp is inited
        # when using model parallel. We have to init model after ddp init
        self.instantiate_model()
        self.warmstart()
        self.instantiate_loss()
        self.instantiate_metrics()
    ...
    def instantiate_datamodule(self):
        ...
        self._datamodule: LightningDataModule = hydra.utils.instantiate(
            self.cfg.datamodule,
            tensor_model_parallel_size=self.cfg.trainer.strategy.tensor_model_parallel_size,
        )
        self._datamodule.prepare_data()
        self._datamodule.setup()
        OmegaConf.clear_resolver("datamodule")
    ...
    def instantiate_model(self):
        ...
        self.model = hydra.utils.instantiate(
            self.model_cfg, _recursive_=recursive, process_group=process_group
        )
    ...
    def warmstart(self):
        if self.cfg.train.get("warmstart", None) is not None:
            ...
            # shard state dict by model parallel rank
            model_cfg = self.model.config
            tensor_model_world_size = parallel_state.get_tensor_model_parallel_world_size()
            tensor_model_parallel_rank = parallel_state.get_tensor_model_parallel_rank()
            shard_state_dict = shard_state_dict_tp(
                state_dict, model_cfg, tensor_model_world_size, tensor_model_parallel_rank
            )
            load_return = self.model.load_state_dict(shard_state_dict, strict=strict)
            logger.info(load_return)
    ...
    def instantiate_loss(self):
        ...
        process_group = parallel_state.get_tensor_model_parallel_group()
        self.loss_fn = hydra.utils.instantiate(loss_fn_cfg, process_group=process_group)
        loss_fn_val_cfg = self.cfg.train.get("loss_fn_val", loss_fn_cfg)
        self.loss_fn_val = hydra.utils.instantiate(loss_fn_val_cfg, process_group=process_group)
